...
def split_this(df, n_districts, holder, n_split=1, pop_col = "POPULATION", dem_col = "G18GOVD", rep_col = "G18GOVR",  orig_dem=0.5, orig_rep=0.5, score_fn=scorers.abs_diff_parta_fn, n_sample_angles=None, degree_limit = 0, dissolve_check = False):
    # print(n_districts)
    if n_districts != 1:
        # Get district ratios:
        if n_districts % 2 == 0:
        ...
        split_this(df_a, part_a, holder=holder, n_split=n_split+1, pop_col = pop_col, dem_col = dem_col, rep_col = rep_col, orig_dem=orig_dem, orig_rep=orig_rep, n_sample_angles=n_sample_angles, degree_limit=degree_limit, score_fn=score_fn)
        split_this(df_b, part_b, holder=holder, n_split=n_split+1, pop_col = pop_col, dem_col = dem_col, rep_col = rep_col, orig_dem=orig_dem, orig_rep=orig_rep, n_sample_angles=n_sample_angles, degree_limit=degree_limit, score_fn=score_fn)

        
    else:
        holder.append(df)

def split_review(input_df, split, split_cols, pop, d_votes, r_votes, plot_party=False, figsize = (8,6), save=None):
